...
@csrf_exempt  # Exempting CSRF for demonstration; handle with care in production
...
#     if query:
#         products = Product.objects.filter(title__icontains=query)  # Adjust 'name' to your product field
#     else:
...
def search_products(request):
    query = request.GET.get('q')
    if query:
        # Perform the search query on all relevant fields
        ...
        # Use a dictionary to track unique products by combining relevant fields
        unique_products = {}
        
        for product in products:
            # Normalize the fields to lower case and strip any extra whitespace to prevent duplicates
            key = (
                product.title.strip().lower(), 
                product.description.strip().lower(), 
                product.brand.strip().lower(), 
                product.category.strip().lower()
            )
            
            # Add the product to the dictionary if it hasn't been added yet
            if key not in unique_products:
                unique_products[key] = product
        
        # Get the unique products from the dictionary
...
def checkout(request):
    ...
    add = Customer.objects.filter(user=user)
    cart_items = Cart.objects.filter(user=user)
    selected_products = request.POST.getlist('selected_products')  # Get selected products from the form
    amount = 0.0
    shipping_amount = 50.0
    total_amount = 0.0
    ...
    if selected_products:
        cart_items = cart_items.filter(product__id__in=selected_products)  # Filter by selected products
        for p in cart_items:
            temp_amount = p.quantity * p.product.discounted_price
            amount += temp_amount
        total_amount = amount + shipping_amount
...
class ProfileView(View):
   ...
   def get(self, request):
      if 'visited' not in request.session:
            # Set a session variable to track the initial visit
